  ...
  return (
    <div
      ...
      className={`card-container-${id}
        ${estilos.contenedor} flex rounded-2xl shadow-2xl
        ${isActive
    ...
    >
      <div
        className={`card-presentacion card-presentacion-${id} group rounded-2xl flex flex-col md:flex-row overflow-hidden ${
     ...
     card-content-item`}
      >
        <h2 className={`card-title-${id} text-2xl md:text-3xl lg:text-4xl font-bold p-3 md:p-5 sticky top-0 rounded-2xl backdrop-blur-sm z-10 w-full text-center`}>
          {titulo}
        </h2>
        <div className="flex flex-col md:flex-row w-full h-auto gap-3 px-2">
          {items.map((item, index) => (
            <div key={index} className={`${item.colorFondo} rounded-4xl p-4 md:p-5 w-full md:w-1/3 flex items-center flex-col gap-2 md:gap-3 relative shadow-2xl min-h-[300px]`}>
              <span className="text-4xl md:text-5xl text-center">
                <FontAwesomeIcon icon={item.icono} />
              </span>
              <h3 className="text-xl md:text-2xl lg:text-3xl font-bold text-center">{item.titulo}</h3>
              ...
              {item.horarios && (
                <div className="w-full flex flex-wrap items-center gap-2">
                  {item.horarios.map((horario, idx) => (
                    <span key={idx} className="bg-escom-200 border-2 border-escom-sombra-50 rounded-full text-xs px-3 py-1 text-escom-sombra-100/90">
